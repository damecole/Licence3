...
char * texte = loading_text_test("test.txt");

 GtkTextBuffer *pBuffer; //Ptr textView
 GtkTextIter pos;
  GtkTextIter debut; //Début du corps du Mémo dans le TextView
    ...
    //On récupère l'adresse du buffer du TextView
     pBuffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(GtkTextView));
     //On récupère l'origine du buffer
      gtk_text_buffer_get_start_iter(pBuffer, &debut);

//On charge le texte de buffer dans le textView
gtk_text_buffer_insert(pBuffer, &fin, buffer, strlen(*texte))
